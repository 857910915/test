...
public class WindowClient {
		...
		window.addWindowListener(new WindowAdapter() {//退出窗体
			public void windowClosing(WindowEvent e){
				...
				if(socket != null && socket.isConnected()){
					try {
						new StartClient(socket, getName(), 3 + "");
		...
		exit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(socket == null){
					...
					try {
						new StartClient(socket, getName(), "3");//发送信息给服务端
						link.setText("连接");
						exit.setText("已退出");
				...
				if(socket != null && socket.isConnected()){	
					...
					if("".equals(ipString) || "".equals(portClinet)){//判断获取内容是否为空
						...
						try {
							...
							exit.setText("退出");
							new StartClient(socket, getName(), 2 + "");//发送该客户端名称至服务器
	...
	public void sendMsg(){
		...
		if("".equals(messages)){//判断信息是否为空
			...
			try {
				//发送信息
				new StartClient(socket,getName() + "：" + messages,"1");
